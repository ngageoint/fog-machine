...
class MapViewController: UIViewController, MKMapViewDelegate {
    ...
    func testForMultiHgtFiles(observer: Observer, currHgt: Hgt) {
        ...
        if checkForHgtFile(observer.coordinate) {
            ...
            if checkRadiusInOneHgt(observer) {
                ...
                self.addOverlay(image, imageLocation: currHgt.getCoordinate())
                ...
                let image = self.generateViewshedImage(obsResults)
                self.addOverlay(image, imageLocation: hgtGrid.upperLeftHgt.getCoordinate(), hgtGridSize: 1.0)
...
//        let grid = HgtGrid(upperLeftHgt: upperLeftHgt, lowerLeftHgt: lowerLeftHgt, upperRightHgt: upperRightHgt, lowerRightHgt: lowerRightHgt)
//        
//        let x = 1201//1371
//        let y = 1201//1431
//        let radius = 100//300
...
//        observer.generateCoordiantesFromXY(grid.coordinate)
//        
...
//        
//        let image = self.generateViewshedImage(obsResults)
//        self.addOverlay(image, imageLocation: grid.getCoordinate(), hgtGridSize: 1.0)
    ...
    func checkForHgtFile(checkCoordinate: CLLocationCoordinate2D) -> Bool {
        ...
        do {
            ...
            let files = try NSFileManager.defaultManager().contentsOfDirectoryAtPath(folder)
            for file: String in files {
                let hgtCoordinate = parseCoordinate(file.componentsSeparatedByString(".")[0])
    ...
    func addOverlay(image: UIImage, imageLocation: CLLocationCoordinate2D, hgtGridSize: Double = 0.0) {
    ...
    func addAnnotationGesture(gestureRecognizer: UIGestureRecognizer) {
        if gestureRecognizer.state == UIGestureRecognizerState.Began {
            ...
            newObserver.setNewCoordinates(newCoordinates, hgtCoordinate: hgt.getCoordinate())
    ...
    func mapView(mapView: MKMapView, annotationView view: MKAnnotationView, calloutAccessoryControlTapped control: UIControl) {
        if let selectedObserver = retrieveObserver((view.annotation?.coordinate)!) {
            if control == view.rightCalloutAccessoryView {
                ...
                testForMultiHgtFiles(selectedObserver.getObserver(), currHgt: self.hgt)
    ...
    func setupFogEvents() {
        ...
        ConnectionManager.onEvent(Event.StartViewshed){ fromPeerId, object in
            ...
            let image = self.generateViewshedImage(self.viewshedResults)
