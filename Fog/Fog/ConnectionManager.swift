...
//    }
    ...
    public static func sendEventToAll<T: Work>(event: String, timeoutSeconds: Double = 30.0,
        ...
        var hasPeers = false
        var deviceCounter = 1
        let selfWork = workDivider(currentQuadrant: allWorkers.count, numberOfQuadrants: deviceCounter)
        receiptAssurance.add(Worker.getMe().displayName, event: event, work: selfWork, timeoutSeconds: timeoutSeconds)
        ...
        dispatch_barrier_async(self.serialQueue) {
            for peer in peers {
                ...
                let theWork = workDivider(currentQuadrant: allWorkers.count, numberOfQuadrants: deviceCounter)
        ...
        receiptAssurance.updateForReceipt(event, receiver: Worker.getMe().displayName)
    }
